...
{
    public class UnsuspensiRepository : RepositoryBase, IUnsuspensiRepository
    {
        public UnsuspensiRepository(IDbTransaction transaction) : base(transaction)
        {
        }
        ...
        {
            if (suspensi == null)
                throw new ArgumentNullException("unsuspensi");
        ...
        public void Add(Suspensi suspensi)
        {
            ...
            suspensi.Id = Connection.ExecuteScalar<int>(
                "INSERT INTO dbo.unsuspensi_cms(ItemId, IsHidden, IsAttachmentHidden, Judul, Pengumuman, Date, info_type ,data_download, LangID) " +
        ...
        {
            Connection.Execute(
                "DELETE FROM dbo.unsuspensi_cms WHERE id = @id",
                new { id },
                Transaction
            );
        }
        ...
        public Suspensi GetByUmbracoId(string itemId)
        {
            return Connection.Query<Suspensi>(
                "SELECT * FROM dbo.unsuspensi_cms WHERE ItemID = @itemId",
                new { itemId },
                Transaction
            ).FirstOrDefault();
        }
